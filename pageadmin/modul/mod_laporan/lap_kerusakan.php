...
$query= "SELECT *  FROM kerusakan ORDER BY id_kerusakan ASC";
...
if (!empty($query))
	...
	$pdf->Ln();
	$pdf->Cell(0,.6,'SISTEM PAKAR KENDARAAN BERMOTOR ',0,0,'C');
	$pdf->Ln();
	$pdf->SetFont('Arial','B',14);
	$pdf->Cell(0,.6,'METODE CENTRAINTY FAKTOR',0,0,'C');
	$pdf->Ln();
	$pdf->SetFont('Arial','B',12);
	$pdf->Cell(0,.6,'Office : Jl. Tentara Pelajar',0,0,'C');
	$pdf->Ln();
	$pdf->Cell(0,.6,'web : http://SISPAKAR.COM e_mail: sispakar@yahoo.com',0,0,'C');	
	$pdf->Ln();
	$pdf->Ln();
	$pdf->Ln();
	...
	$x=$pdf->GetY();
	$pdf->SetY($x+1);
	$pdf->Cell(0,1,'Daftar Kerusakan Sepeda Motor ',0,0,'C');
	...
	$pdf->Cell(14,1,'KERUSAKAN',1,0,'C');
	$pdf->Cell(14,1,'SOLUSI',1,0,'C');
...
while ($hasil=mysql_fetch_array($baca))
{
	...
	$pdf->Cell(2,1,$no,1,0,'C');
	$pdf->Cell(14,1,$hasil['nm_kerusakan'],1,0,'C');
	$pdf->Cell(14,1,$hasil['solusi'],1,0,'C');
	...
	$pdf->Ln();
	$pdf->Cell(15,0.5,'');
	$pdf->Cell(0,0.5,'SISTEM PAKAR ',0,0,'C');
	$pdf->Ln();
	$pdf->Ln();
	$pdf->Ln();
	$pdf->Ln();
